package com.joek.nuxtube.extracters.youtubeExtractor;
...
public abstract class YouTubeExtractor {
    ...
    public void extract(String youtubeLink) {
        TaskRunner.getInstance().execute(() -> doInBackground(youtubeLink));
    }

    protected abstract void onExtractionComplete( SparseArray<YtFile> ytFiles, @Nullable VideoMeta videoMeta);


    private void doInBackground(String link) {
        videoID = null;
        if (link == null) {
            return;
        }
        Matcher mat = patYouTubePageLink.matcher(link);
        if (mat.find()) {
            videoID = mat.group(3);
        } else {
            mat = patYouTubeShortLink.matcher(link);
            if (mat.find()) {
                videoID = mat.group(3);
            } else if (link.matches("\\p{Graph}+?")) {
                videoID = link;
            }
        }

        try {
            Pair<SparseArray<YtFile>, SparseArray<String>> pair = getStreamUrls();
            decipherSignature(pair.second, pair.first);
        } catch (IOException | InterruptedException | JSONException e) {
            e.printStackTrace();
        }
    }

    private Pair<SparseArray<YtFile>, SparseArray<String>> getStreamUrls() throws IOException, InterruptedException, JSONException {
        ...
        if (mat.find()) {
            ...
            if (!streamingData.isNull("formats")){
            ...
            for (int i = 0; i < adaptiveFormats.length(); i++) {
                ...
                if (FORMAT_MAP.get(itag) != null) {
                    ...
                    //Log.e(LOG_TAG, "itag = " + String.valueOf(itag));
                }
            }

            JSONObject videoDetails = ytPlayerResponse.getJSONObject("videoDetails");
            JSONObject microFormats = ytPlayerResponse.getJSONObject("microformat").getJSONObject("playerMicroformatRenderer");

//            Iterator<String> x = videoDetails.keys();
//
//            while (x.hasNext()){
...
//            }
            this.videoMeta = new VideoMeta(videoDetails.getString("videoId"),
                    ...
                    microFormats.getString("publishDate"));
        ...
        if (encSignatures.size() > 0) {
            ...
            if (mat.find()) {
                ...
                Log.e(LOG_TAG, decipherJsFileName);
            }
        ...
        return new Pair<>(ytFiles, encSignatures);
    }

    private boolean decipherSignature(final SparseArray<String> encSignatures, SparseArray<YtFile> ytFileSparseArray) throws IOException {
        ...
        if (decipherFunctionName == null || decipherFunctions == null) {
            ...
            if (mat.find()) {
                ...
                decipherViaWebView(encSignatures, ytFileSparseArray);
                if (CACHING) {
    ...
    private void readDecipherFunctFromCache() {
        Log.d(LOG_TAG, "reading from cache.");
    ...
    private void writeDeciperFunctToChache() {
        Log.d(LOG_TAG, "writing to cache.");
        File cacheFile = new File(cacheDirPath + "/" + CACHE_FILE_NAME);
    ...
    private void decipherViaWebView(final SparseArray<String> encSignatures, SparseArray<YtFile> ytFileSparseArray) {
        ...
        new Handler(Looper.getMainLooper()).post(() -> {
            ...
            webView.evaluateJavascript(stb.toString(), s -> {

                String pattern = Pattern.quote("\\" + "n");
                String[] sigs = s.split(pattern);

                for (int i = 0; i < encSignatures.size() && i < sigs.length; i++) {
                    int key = encSignatures.keyAt(i);
                    String url = ytFileSparseArray.get(key).getUrl();
                    url += "&sig=" + sigs[i];
                    YtFile newFile = new YtFile(FORMAT_MAP.get(key), url);
                    ytFileSparseArray.put(key, newFile);
                }
                onExtractionComplete(ytFileSparseArray, this.videoMeta);
            });
        });
    }
}
